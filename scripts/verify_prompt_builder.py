#!/usr/bin/env python3
"""Verification script for Day 5 Step 3: Prompt Builder for AI Research Assistant."""
...
def main() -> None:
    """Run verification for PromptBuilder across 5 benchmark query intents."""
    print("\n============================================================")
    print(" DAY 5 STEP 3: PROMPT BUILDER VERIFICATION")
    print("============================================================\n")
    ...
    for label, query_str in benchmark_queries:
        ...
        request = PromptRequest(
            ...
            max_context_tokens=150,  # Constrained limit to test context selection and exclusion
        )
        response = prompt_builder.build_prompt(request)

        print("-" * 60)
        print(f"[{label}] Query: '{query_str}'")
        print(f"Intent Detected: {analysis.intent.value.upper()}")
        print(f"Template Name:  {response.metadata.template_name}")
        print(f"Total Tokens:   {response.estimated_tokens}")
        print(f"Coverage Ratio: {response.context_coverage:.2%}")
        print(f"Included Chunks ({len(response.included_chunks)}): {[c.chunk_id for c in response.included_chunks]}")
        print(f"Excluded Chunks ({len(response.excluded_chunks)}): {[c.chunk_id for c in response.excluded_chunks]}")
        print("\n--- ASSEMBLED FULL PROMPT SNIPPET ---")
        prompt_lines = response.full_prompt.split("\n")
        snippet = "\n".join(prompt_lines[:25])
        print(snippet)
        if len(prompt_lines) > 25:
            print("... [Remaining prompt lines truncated for display] ...")
        print("-" * 60 + "\n")
